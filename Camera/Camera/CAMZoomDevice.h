...
/// Sets the camera's zoom factor to the given value as fast as possible. \c zoomFactor should be
/// in the range <tt>[minZoomFactor, maxZoomFactor]</tt>.
///
...
/// an exponential rate, with a \c rate of \c 1 meaning it will double/halve every second.
/// \c zoomFactor should be in the range <tt>[minZoomFactor, maxZoomFactor]</tt>.
///
/// Returned signal sends the new \c zoomFactor and completes when the new zoom factor is set, or
/// errs if there is a problem setting the zoom factor. All events are sent on an arbitrary thread.
