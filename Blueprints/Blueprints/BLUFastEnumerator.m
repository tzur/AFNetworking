...
#pragma mark -
#pragma mark BLUFastEnumeratorState
#pragma mark -
...
/// that are returned are placed on the stack and are \c __usafe_unretained, so they must be
/// retained.
@interface BLUFastEnumeratorState : NSObject {
@public
...
+ (instancetype)stateForEnumerator:(BLUFastEnumerator *)enumerator;
...
@property (weak, nonatomic) BLUFastEnumerator *enumerator;
...
#pragma mark -
#pragma mark BLUFastEnumerator
#pragma mark -

@implementation BLUFastEnumeratorState

- (instancetype)initWithEnumerator:(BLUFastEnumerator *)enumerator {
  if (self = [super init]) {
...
+ (instancetype)stateForEnumerator:(BLUFastEnumerator *)enumerator {
  ...
  BLUFastEnumeratorState *state = objc_getAssociatedObject(enumerator, key);
  if (state) {
    return state;
  }

  BLUFastEnumeratorState *newState = [[BLUFastEnumeratorState alloc] initWithEnumerator:enumerator];
...
typedef NS_ENUM(unsigned long, BLUEnumerationState) {
  /// Enumeration has not been started yet.
  BLUEnumerationStateInitial = 0,
  /// Enumeration has been started.
  BLUEnumerationStateStarted
};
...
typedef NSUInteger (^BLUFastEnumeratorOperationBlock)(NSFastEnumerationState *state,
                                                      __unsafe_unretained id sourceItems[],
                                                      __strong id outputItems[],
                                                      NSUInteger outputItemsCount);

@interface BLUFastEnumerator ()

/// Operation to perform on \c source.
@property (copy, nonatomic, nullable) BLUFastEnumeratorOperationBlock operation;

@end

@implementation BLUFastEnumerator
...
- (instancetype)initWithSource:(id<NSFastEnumeration>)source
                     operation:(nullable BLUFastEnumeratorOperationBlock)operation {
  if (self = [super init]) {
    _source = source;
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  BLUFastEnumeratorState *ourState = [BLUFastEnumeratorState stateForEnumerator:self];
  ...
  do {
    ...
    if (state->extra[kSourceCountKey] == state->extra[kSourceIndexKey]) {
      NSUInteger sourceCount = [self.source countByEnumeratingWithState:state objects:buffer
                                                                  count:len];
      state->extra[kSourceCountKey] = sourceCount;
      state->extra[kSourceIndexKey] = 0;
    }
...
- (BLUFastEnumerator *)map:(BLUFastMapEnumerationBlock)block {
  BLUFastEnumeratorOperationBlock map = ^NSUInteger(NSFastEnumerationState *state,
                                                    __unsafe_unretained id sourceItems[],
                                                    __strong id outputItems[],
                                                    NSUInteger outputItemsCount) {
    NSUInteger count = 0;
  ...
  return [[BLUFastEnumerator alloc] initWithSource:self operation:map];
}
...
- (BLUFastEnumerator *)flatMap:(BLUFastFlatMapEnumerationBlock)block {
  ...
  BLUFastEnumeratorOperationBlock flatMap = ^NSUInteger(NSFastEnumerationState *state,
                                                        __unsafe_unretained id sourceItems[],
                                                        __strong id outputItems[],
                                                        NSUInteger outputItemsCount) {
    NSUInteger count = 0;
  ...
  return [[BLUFastEnumerator alloc] initWithSource:self operation:flatMap];
}
...
@implementation NSArray (BLUFastEnumerator)

- (BLUFastEnumerator *)blu_enumerator {
  return [[BLUFastEnumerator alloc] initWithSource:self];
}

@end

@implementation NSOrderedSet (BLUFastEnumerator)
