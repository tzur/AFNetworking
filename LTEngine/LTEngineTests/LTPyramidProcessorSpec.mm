...
context(@"small inputs", ^{
  it(@"should create correct pyramid for 1x1 image", ^{
    auto input = [LTTexture byteRGBATextureWithSize:CGSizeMake(1, 1)];
    auto outputs = [LTPyramidProcessor levelsForInput:input];
  ...
  it(@"should create correct pyramid for 2x3 image", ^{
    auto input = [LTTexture byteRGBATextureWithSize:CGSizeMake(2, 3)];
    auto outputs = [LTPyramidProcessor levelsForInput:input];
    expect(outputs.count).to.equal(1);
  ...
  it(@"should create correct pyramid for 5x4 image", ^{
    auto input = [LTTexture byteRGBATextureWithSize:CGSizeMake(5, 4)];
    auto outputs = [LTPyramidProcessor levelsForInput:input];
    expect(outputs.count).to.equal(1);
