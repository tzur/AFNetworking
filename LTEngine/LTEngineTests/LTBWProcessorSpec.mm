...
context(@"processing", ^{
  ...
  it(@"should process brightness correctly", ^{
    cv::Mat4b expectedOutput(2, 2, cv::Vec4b(217, 217, 217, 255));
    processor = [[LTBWProcessor alloc] initWithInput:inputTexture output:outputTexture];
  ...
  it(@"should cover alpha with frame when frame is opaque", ^{
    inputTexture = [LTTexture byteRGBATextureWithSize:CGSizeMake(2, 2)];
    [inputTexture clearWithColor:LTVector4(0.5, 0.5, 0.5, 0.0)];
    processor = [[LTBWProcessor alloc] initWithInput:inputTexture output:outputTexture];
  ...
  it(@"should not cover alpha with frame, when frame is clear", ^{
    inputTexture = [LTTexture byteRGBATextureWithSize:CGSizeMake(2, 2)];
    [inputTexture clearWithColor:LTVector4(0.5, 0.5, 0.5, 0.0)];
    processor = [[LTBWProcessor alloc] initWithInput:inputTexture output:outputTexture];

    cv::Mat4b expectedOutput = inputTexture.image;
    // Only the first channel should be used.
    [processor.frameTexture clearWithColor:LTVector4(0.5, 1, 1, 1)];
    [processor process];
