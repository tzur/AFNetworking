...
- (cv::Mat1f)reducedSumOfMaskForDimension:(LTDecimationDimension)dimension {
  ...
  float maxMaskPixelValue = (self.mask.dataType == LTGLPixelDataType8Unorm) ? UCHAR_MAX : 1;
...
- (void)process {
  ...
  [self.auxiliaryFbo clearColor:LTVector4::zeros()];
  ...
  [self.outputFbo clearColor:LTVector4::zeros()];
