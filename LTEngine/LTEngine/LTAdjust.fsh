...
uniform sampler2D lowResolutionSourceTexture;
...
void main() {
  ...
  mediump vec4 smoothColor = texture2D(lowResolutionSourceTexture, vTexcoord);
  ...
  mediump float smoothLum = dot(smoothColor.rgb, kRGBToYPrime);
  ...
  lum = mix(lum, mix(-0.4 * (details - 3.5 * lum), details, step(0.0, detailsBoost)),
                          abs(detailsBoost));
  // Sharpen.
  mediump float highFrequency = originalLum - smoothLum;
  mediump float sharpenMask = smoothstep(0.005, 0.03, abs(highFrequency));
  lum += sharpen * sharpenMask * highFrequency;
