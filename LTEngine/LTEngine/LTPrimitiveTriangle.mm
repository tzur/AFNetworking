...
Triangle::Triangle(const LTTriangleCorners &corners) noexcept {
  triangleType = LTTypeForCorners(corners);
  v = LTUpdatedCornersForType(corners, triangleType);
}
...
BOOL Triangle::containsPoint(CGPoint point) const noexcept {
  switch (triangleType) {
    ...
    case Triangle::Type::Clockwise:
      return LTNonDegenerateTriangleContainsPoint(point, v, YES);
    case Triangle::Type::CounterClockwise:
      return LTNonDegenerateTriangleContainsPoint(point, v, NO);
  }
}
