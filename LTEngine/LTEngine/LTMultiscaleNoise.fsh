...
// Higher values will create higher number of island-like structures. Recommended values are in
// [2-20] range.
...
highp float bilinearNoise(highp vec2 p) {
  ...
  // Bilinear interpolation: x-axis.
	highp float top = mix(noise(i), noise(i + vec2(1.0, 0.0)), f.x);
	highp float bottom = mix(noise(i + vec2(0.0, 1.0)), noise(i + vec2(1.0, 1.0)), f.x);
  // Bilinear interpolation: y-axis. Subtract mean and center around zero.
	return mix(top, bottom, f.y) - 0.5;
}

void main() {
  sourceTexture;
  highp vec2 p = vTexcoord.xy * vec2(density * directionality, density);
  // Add zero-mean noise at different scales to DC(0.5).
  // In order to create a multiscale noise start by bilinearly sampling nearby values to create a
  // smoothly changing signal and add progressively higher-frequencies on top of it by sampling
  // signal at each scale further away from each other. Additionally, tones down the contribution of
  // the high scales.
