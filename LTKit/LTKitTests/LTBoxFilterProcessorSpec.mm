...
beforeEach(^{
  ...
  LTGLTexture *input = [[LTGLTexture alloc] initWithImage:delta];
...
it(@"should process input image correctly", ^{
  ...
  processedDelta = cv::Vec4b(5, 5, 5, 255); // 5.2041

  expect(LTFuzzyCompareMat(processedDelta, [output image], 1)).to.beTruthy();
});
