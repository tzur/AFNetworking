...
/// Updates the buffer with the data in the given array (of \c NSData), concated one after another
/// in according to their order in the array. If the total size of the data is different than the
/// current size of the buffer, the buffer will be re-allocated.
///
/// @note buffers of type \c LTArrayBufferUsageStaticDraw should only be initialized once. Trying to
/// update will yield an \c LTGLException with \c
/// kLTArrayBufferDisallowsStaticBufferUpdateException. For a buffer than can be updated
