...
///
/// @important current implementation only supports RGBA8 images, without transparency. The alpha
/// channel will not be compressed and the descompressed images will contain an opaque alpha
/// channel.
///
...
/// populated.
///
/// @important the ImageZero file format currently does not support alpha channels. Therefore, its
/// data won't be stored to disk.
///
/// @important image width and height cannot be larger than 2^16 - 1.
- (BOOL)compressImage:(const cv::Mat4b &)image toPath:(NSString *)path error:(NSError **)error;
...
///
/// @important the ImageZero file format currently does not support alpha channels. Therefore, the
/// returned image will have a constant opaque alpha channel.
- (BOOL)decompressFromPath:(NSString *)path toImage:(cv::Mat4b *)image error:(NSError **)error;
...
/// Maximal compressed ImageZero file size for the given \c image.
