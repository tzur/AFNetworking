...
uniform highp vec2 outputTextureSize;
...
void main() {
  // Sample only if mod(x + y, 2) != 0.
  highp vec2 inputPixelCoord = floor(vSampleCoords[0] * inputTextureSize);
  bool shouldSample = mod(inputPixelCoord.x + inputPixelCoord.y, 2.0) > 0.0;

  highp vec2 outputPixelCoord = vSampleCoordInput * outputTextureSize;
  bool writeOriginal = !all(lessThan(mod(outputPixelCoord, 2.0), vec2(1.0)));

  if (!sameSize && writeOriginal) {
