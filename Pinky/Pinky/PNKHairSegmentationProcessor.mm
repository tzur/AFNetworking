...
@interface PNKHairSegmentationProcessor ()
...
@implementation PNKHairSegmentationProcessor
...
- (nullable instancetype)initWithNetworkModel:(NSURL *)networkModelURL
  ...
  if (self = [super init]) {
    ...
    _dispatchQueue = dispatch_queue_create("com.lightricks.Pinky.HairSegmentationProcessor",
...
- (CGSize)outputSizeWithInputSize:(CGSize)size {
  static const NSUInteger kInputSmallSide = 256;
  static const NSUInteger kSizeRequiredDivisor = 8;
  if (size.width > size.height) {
    int width = (size.width * kInputSmallSide) / size.height;
    width = ((width + (kSizeRequiredDivisor - 1)) / kSizeRequiredDivisor) * kSizeRequiredDivisor;
    return CGSizeMake(width, kInputSmallSide);
  } else {
    int height = (size.height * kInputSmallSide) / size.width;
    height = ((height + (kSizeRequiredDivisor - 1)) / kSizeRequiredDivisor) * kSizeRequiredDivisor;
